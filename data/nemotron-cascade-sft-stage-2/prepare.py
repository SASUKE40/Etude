...
class RollingParquetWriter:
    def __init__(self, output_dir, split, rows_per_shard):
        self.split = split
        ...
        self.writer = None
        self.buffer = []
        self.flush_rows = min(rows_per_shard, 1024)

    def _open_writer(self):
        path = os.path.join(self.split_dir, f"part_{self.shard_idx:05d}.parquet")
        self.writer = pq.ParquetWriter(path, PARQUET_SCHEMA, compression="zstd")
...
def prepare(
    ...
    writers = {
        "train": RollingParquetWriter(output_dir, "train", rows_per_shard),
        "val": RollingParquetWriter(output_dir, "val", rows_per_shard),
    }
    skipped = 0
    counts = {
