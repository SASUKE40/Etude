...
# Override with ETUDE_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("ETUDE_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via ETUDE_DTYPE={env}"
