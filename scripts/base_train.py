...
# Model architecture
parser.add_argument("--depth", type=int, default=24, help="number of layers (must be divisible by group_size)")
parser.add_argument("--n-embd", type=int, default=1024, help="hidden dimension")
parser.add_argument("--max-seq-len", type=int, default=2048, help="max context length for training")
parser.add_argument("--mtp-steps", type=int, default=1, help="number of multi-token prediction steps (0=disabled)")
...
def build_model_meta(depth, n_embd=None):
...
if args.fp8:
    if device_type != "cuda":
        ...
        from etude.fp8 import Float8LinearConfig, convert_to_float8_training
...
# Our reference model is d24 (the standard Etude architecture)
d_ref = build_model_meta(24, n_embd=1024) # creates the model on meta device
D_REF = args.target_param_data_ratio * get_scaling_params(d_ref) # compute-optimal training horizon in tokens
B_REF = 2**19 # optimal batch size ~= 524,288 tokens (measured empirically)
...
if batch_ratio != 1.0:
    # SGD: linear scaling with batch size is standard (not used in etude)
