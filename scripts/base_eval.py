...
Examples:
    ...
    # Evaluate a etude model (e.g. d24) using 8 GPUs
...
from etude.common import compute_init, compute_cleanup, print0, get_base_dir, autodetect_device_type, download_file_with_lock
from etude.tokenizer import HuggingFaceTokenizer, get_token_bytes
from etude.checkpoint_manager import load_model
from etude.core_eval import evaluate_task
from etude.dataloader import tokenizing_distributed_data_loader_bos_bestfit
...
class ModelWrapper:
    """Lightweight wrapper to give HuggingFace models a etude-compatible interface."""
...
def main():
    ...
    parser.add_argument('--model-tag', type=str, default=None, help='etude model tag to identify the checkpoint directory')
    ...
    if is_hf_model:
        ...
        token_bytes = get_token_bytes(device=device, vocab_size=meta["model_config"]["vocab_size"])
